...
def main():
    ...
    async def on_command_error(ctx: Context, error: CommandError):
        ...
        if isinstance(error, UserInputError):
            await ctx.send(f"400 Bad Request: Some part of your input was incorrect, either too few arguments, too many arguments or invalid arguments. Have a look at `>help {ctx.invoked_with}`")
            return
        ...
        if isinstance(error, CheckFailure):
            if isinstance(error, OnlyAdminCommands):
                ...
                await ctx.send("403 Forbidden: You are for some reason (probably a good one) not permitted to execute this command.")
            return
        ...
        if isinstance(error, CommandInvokeError):
            await ctx.send("500 Internal Server Error: Uh oh, something went wrong on our side while processing that command.")
        else:
